...
void LandscapeMgr::init()
{
    ...
    landscapeCache.setMaxCost(conf->value("landscape/cache_size_mb", 10).toInt());
...
//			QString hPa = qc_("hPa", "pressure");//bool LandscapeMgr::setDefaultLandscapeID(const QString& id)
//{
////	if (id.isEmpty())
////		return false;
////	defaultLandscapeID = id;
...
Landscape* LandscapeMgr::createFromFile(const QString& landscapeFile, const QString& landscapeId)
{
	...
	if (s=="old_style")
        landscape = new LandscapeSpherical();//landscape = new LandscapeOldStyle();
	else if (s=="spherical")
