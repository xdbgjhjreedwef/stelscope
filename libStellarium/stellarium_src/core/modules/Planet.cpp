...
//	}
    ...
    //TODO: remove unneeded models, normals
...
//{
//	if(!texName.isEmpty())//QString Planet::getNameI18n() const
//{
//	if (!iauMoonNumber.isEmpty())
//		return QString("%1 (%2)").arg(nameI18, iauMoonNumber);
//	else
...
QString Planet::getEnglishName() const
{
	if (!iauMoonNumber.isEmpty())
        ...
        return englishName;//QString Planet::getNameI18n() const
    //{
    //	if (!iauMoonNumber.isEmpty())
    //		return QString("%1 (%2)").arg(nameI18, iauMoonNumber);
    //	else
...
QString Planet::getInfoStringAbsoluteMagnitude(const StelCore *core, const InfoStringGroup& flags) const
{
	...
	{
        oss << QString("%1: %2<br/>").arg(("Absolute Magnitude")).arg(getAbsoluteMagnitude(), 0, 'f', 2);
		const float moMag=getMeanOppositionMagnitude();
		if (moMag<50.f)
            oss << QString("%1: %2<br/>").arg(("Mean Opposition Magnitude")).arg(moMag, 0, 'f', 2);
	}
...
QString Planet::getInfoString(const StelCore* core, const InfoStringGroup& flags) const
{
	...
	if (flags&ObjectType && getPlanetType()!=isUNDEFINED)
	{
		if (getPlanetType()==isComet)
		{
			const QString cometType = (static_cast<KeplerOrbit*>(orbitPtr)->getEccentricity() < 1.0) ?
                        QString("periodic").append("type of comet") :
                        QString("non-periodic").append("type of comet");
            oss << QString("%1: <b>%2</b> (%3)<br/>").arg(("Type"), (getPlanetTypeString()), cometType);
		}
		else		
            oss << QString("%1: <b>%2</b><br/>").arg(("Type"), (getPlanetTypeString()));
	}
	...
	if (flags&EclipticCoordJ2000)
	{
        ...
        oss << QString("%1 XYZ J2000.0 (%2) without aberration: %3/%4/%5 AU").arg(QString("Ecliptical").append("coordinates"), algoName, QString::number(eclipticPos[0], 'f', 7), QString::number(eclipticPos[1], 'f', 7), QString::number(eclipticPos[2], 'f', 7)) << "<br>";
		Vec3d eclAb=eclipticPos+aberrationPush;
        oss << QString("%1 XYZ J2000.0 (%2) with aberration: %3/%4/%5 AU").arg(QString("Ecliptical").append("coordinates"), algoName, QString::number(eclAb[0], 'f', 7), QString::number(eclAb[1], 'f', 7), QString::number(eclAb[2], 'f', 7)) << "<br>";
	}
#endif
	...
	if (flags&ProperMotion && !core->getCurrentObserver()->isObserverLifeOver())
	{
		...
		if (pa<0) pa += 2.*M_PI;
        oss << QString("%1: %2 %3 %4%5<br/>").arg(("Hourly motion"), StelUtils::radToDmsStr(deltaEq),QString("towards").append("into the direction of"), QString::number(pa*M_180_PI, 'f', 1), QChar(0x00B0));
        oss << QString("%1: d&alpha;=%2 d&delta;=%3<br/>").arg(("Hourly motion"), StelUtils::radToDmsStr(ra_equ-ra_equPrev), StelUtils::radToDmsStr(dec_equ-dec_equPrev));
//        oss << QString("%1: %2 %3 %4%5<br/>").arg(("Hourly motion"), withDecimalDegree ? StelUtils::radToDecDegStr(deltaEq) : StelUtils::radToDmsStr(deltaEq),QString("towards").append("into the direction of"), QString::number(pa*M_180_PI, 'f', 1), QChar(0x00B0));
//        oss << QString("%1: d&alpha;=%2 d&delta;=%3<br/>").arg(("Hourly motion"), withDecimalDegree ? StelUtils::radToDecDegStr(ra_equ-ra_equPrev) : StelUtils::radToDmsStr(ra_equ-ra_equPrev), withDecimalDegree ? StelUtils::radToDecDegStr(dec_equ-dec_equPrev) : StelUtils::radToDmsStr(dec_equ-dec_equPrev));
	...
	if (flags&Distance)
	{
        ...
        QString au = QString("AU").append("distance, astronomical unit");
		// TRANSLATORS: Unit of measure for distance - kilometers
        QString km = QString("km").append("distance");
		...
		if (englishName!="Sun")
		{
			...
			else
			{
                ...
                km = QString("M km").append("distance");
			}

            oss << QString("%1: %2 %3 (%4 %5)<br/>").arg(("Distance from Sun"), distAU, au, distKM, km);
		}
        ...
        oss << QString("%1: %2 %3 (%4 %5)<br/>").arg(("Distance"), distAU, au, distKM, km);
		// TRANSLATORS: Distance measured in terms of the speed of light
        oss << QString("%1: %2 <br/>").arg(("Light time"), StelUtils::hoursToHmsStr(distanceKm/SPEED_OF_LIGHT/3600.) );
	...
	if (flags&Velocity)
	{
        ...
        QString kms = QString("km/s").append("speed");
		...
		{ // AU/d * km/AU /24
            ...
            oss << QString("%1: %2 %3<br/>").arg(("Orbital velocity")).arg(orbVelKms, 0, 'f', 3).arg(kms);
			const double helioVel=getHeliocentricEclipticVelocity().length();
			if (!fuzzyEquals(helioVel, orbVel))
                oss << QString("%1: %2 %3<br/>").arg(("Heliocentric velocity")).arg(helioVel* AU/86400., 0, 'f', 3).arg(kms);
		}
	...
	{
        oss << ("NOTE: orbital elements outdated -- consider updating!") << "<br/>";
	}
...
//			else
            ...
            sizeStr = QString("%1, %2: %3").arg(s1, ("with rings"), s2);
		}
		else
		{
...
//				else
                ...
                sizeStr = QString("%1, %2: %3").arg(s1, ("scaled up to"), s2);
			}
			else
			{
//				if (withDecimalDegree)
...
//				else
        ...
        oss << QString("%1: %2<br/>").arg(("Apparent diameter"), sizeStr);
	}

	if (flags&Size)
	{
        QString diam = (getPlanetType()==isPlanet ? ("Equatorial diameter") : ("Diameter")); // Many asteroids have irregular shape (Currently unhandled)
        oss << QString("%1: %2 %3<br/>").arg(diam, QString::number(AU * getEquatorialRadius() * 2.0, 'f', 1) , QString("km").append("distance"));
...
//		else
		{
            oss << QString("%1: %2<br/>").arg(("Elongation"), elo);
            oss << QString("%1: %2<br/>").arg(("Elong. in Ecl.Long."), dLam);
            oss << QString("%1: %2<br/>").arg(("Phase angle"), pha);
			if (withIllum)
                oss << QString("%1: %2%<br/>").arg(("Illuminated"), QString::number(getPhase(observerHelioPos) * 100., 'f', 1));
		}
...
//			else
            ...
            oss << QString("%1 %2 &mdash; %3: %4<br/>").arg(("Angular distance"), "getNameI18n()", "this->parent->getNameI18n()", ad);
		}
	}
	return str;
}
...
//		if (withTables)
//			oss << "<table style='margin:0em 0em 0em -0.125em;border-spacing:0px;border:0px;'>";
        //const QByteArray fmt=withTables ? "<tr><td>%1:</td><td style='text-align: right;'> %2</td><td> %3 (%4 a)</td></tr>"
          //                              : "%1: %2 %3 (%4 a)<br/>";
        const QByteArray fmt= "%1: %2 %3 (%4 a)<br/>";
		// TRANSLATORS: Unit of measure for period - days
        QString days = QString("days").append("duration");
		...
		if ((siderealPeriod>0.0) && (celestialObject != "Sun"))
		{
            ...
            oss << QString(fmt).arg(("Sidereal period"), QString::number(siderealPeriod, 'f', 2), days, QString::number(siderealPeriod/365.25, 'f', 3));
		}
		...
		if (siderealPeriodCurrentPlanet > 0.0 && siderealPeriod > 0.0 && currentPlanet->getPlanetType()==Planet::isPlanet && (getPlanetType()==Planet::isPlanet || currentPlanet->getEnglishName()==celestialObject))
		{
            ...
            oss << QString(fmt).arg(("Synodic period"), QString::number(synodicPeriod, 'f', 2), days, QString::number(synodicPeriod/365.25, 'f', 3));
		}
//		if (withTables)
//			oss << "</table>";
	}
	...
	double &lngDeg, double &altitude, double &pathWidth, double &duration, double &magnitude)
{
	...
	if (p > 0.) // Central eclipse : Moon's shadow axis is touching Earth
	{
        ...
        // Path width in flagShowDecimalDegreeskilometers
	...
	else  // Partial eclipse or non-central eclipse
	{
		...
		const double cd1 = sqrt(1.- e2) * cos(d) / rho1;
		const double rho2 = sqrt(1.- e2 * sin(d) * sin(d));
        //const doubleflagShowDecimalDegrees sd1d2 = e2 * sin(d) * cos(d) / (rho1 * rho2);
...
QString Planet::getInfoStringExtra(const StelCore *core, const InfoStringGroup& flags) const
{
	...
	if (flags&Extra)
	{
        ...
        QString mps = QString("m/s").append("speed");
		...
		if (siderealPeriod>0.0)
		{
			if (qAbs(siderealDay)>0 && getPlanetType()!=isArtificial)
			{
                ...
                const QByteArray fmt="%1: %2<br/>";

//				if (withTables)
//					oss << "<table style='margin:0em 0em 0em -0.125em;border-spacing:0px;border:0px;'>";
                oss << QString(fmt).arg(("Sidereal day"), StelUtils::hoursToHmsStr(qAbs(siderealDay*24)));
				if (englishName!="Sun")
                    oss << QString(fmt).arg(("Mean solar day"), StelUtils::hoursToHmsStr(qAbs(getMeanSolarDay()*24)));
//				if (withTables)
//					oss << "</table>";
			}
			else if (re.period==0.)
			{
                oss << ("The period of rotation is chaotic") << "<br />";
			}
			if (qAbs(re.W1)>0.)
			{
				const double eqRotVel = (2.0*M_PI*AU/(360.*86400.0))*getEquatorialRadius()*re.W1;
				if (eqRotVel>1.)
                    oss << QString("%1: %2 %3<br/>").arg(("Equatorial rotation velocity")).arg(qAbs(eqRotVel), 0, 'f', 3).arg(kms);
				else
                    oss << QString("%1: %2 %3<br/>").arg(("Equatorial rotation velocity")).arg(qAbs(eqRotVel*1000.), 0, 'f', 3).arg(mps);
			}
		...
		if (englishName=="Moon" && onEarth)
		{
			...
			if (deltaLong<0.5 || deltaLong>359.5)
                moonPhase = QString("New Moon, Moon phase");
			else if (deltaLong<89.5)
                moonPhase = "Waxing Crescent, Moon phase";
			else if (deltaLong<90.5)
                moonPhase = "First Quarter, Moon phase";
			else if (deltaLong<179.5)
                moonPhase = "Waxing Gibbous, Moon phase";
			else if (deltaLong<180.5)
                moonPhase = "Full Moon, Moon phase";
			else if (deltaLong<269.5)
                moonPhase = "Waning Gibbous, Moon phase";
			else if (deltaLong<270.5)
                moonPhase = "Third Quarter, Moon phase";
			else if (deltaLong<359.5)
                moonPhase = "Waning Crescent, Moon phase";
			else
			{
            ...
            oss << QString("%1: %2 %3").arg(("Moon age"), QString::number(age, 'f', 1), ("days old"));
...
//			}
//			else
                oss << QString("%1: %2<br/>").arg(("Position angle of bright limb"), chiStr);
...
//			else
			{
                oss << QString("%1: %2<br/>").arg(("Position Angle of axis"), paAxisStr);
                oss << QString("%1: %2 %3 %4 (%5)<br/>").arg(("Libration"), totalLibrationStr, ("towards"), librationAngleStr, limbStr);
                oss << QString("%1: %2/%3<br/>").arg(("Libration"), libLStr, libBStr);
                oss << QString("%1: %2/%3<br/>").arg(("Subsolar point"), subsolarLStr, subsolarBStr);
                oss << QString("%1: %2<br/>").arg(("Colongitude"), colongitudeStr);
			}
		}
...
//			{
//				paAxisStr=StelUtils::radToDecDegStr(ssop.first[3], 1);
//				subearthLStr=StelUtils::radToDecDegStr(Le, 1)//			if (withTables)
            //			{
            //				oss << "<table style='margin:0em 0em 0em -0.125em;border-spacing:0px;border:0px;'>";
            //				oss << QString("<tr><td colspan=\"2\">%1:</td><td align=\"right\"> %2</td></tr>").arg(q_("Position Angle of axis"), paAxisStr);
...
//			else
			{
                oss << QString("%1: %2<br/>").arg(("Position Angle of axis"), paAxisStr);
                oss << QString("%1: L<sub>%2e</sub>=%3 &phi;<sub>e</sub>: %4<br/>").arg(("Center point"),   lngSystem, subearthLStr, subearthBStr);
                oss << QString("%1: L<sub>%2s</sub>=%3 &phi;<sub>s</sub>: %4<br/>").arg(("Subsolar point"), lngSystem, subsolarLStr, subsolarBStr);
			}
		}
		...
		if (englishName=="Sun")
		{
			...
			if (eclipseObscuration>1.e-7) // needed to avoid false display of 1e-14 or so.
			{
				...
				if (onEarth && obj == ssystem->getMoon())
				{
					const double eclipseMagnitude =
							...
							/ angularSize;
                    oss << QString("%1: %2<br />").arg(("Eclipse magnitude"), QString::number(eclipseMagnitude, 'f', 3));
				}
                oss << QString("%1: %2%<br />").arg(("Eclipse obscuration"), QString::number(eclipseObscuration, 'f', 2));
			}
			...
			if (onEarth)
			{
				...
				if (raDiff < 3. || raDiff > 357.)
				{
					...
					{
						oss << QString("%1: %2 ").arg(
                                 ("Moon/Sun diameter ratio"), // It seems magnitude of total/annular eclipses sometimes represented by this value
						...
						oss << QString("%1: %2%3 %4%5<br/>").arg(
                                 ...
                                 ("s"));
                        QString info = ("Center of solar eclipse (Lat./Long.)");
//						if (withDecimalDegree)
...
//						else
						...
						oss << QString("%1 %2 %3 %4°<br/>").arg(
                                 ("Shadow center point is"),
								 QString::number(distance, 'f', 1),
                                 ("km towards azimuth"),
								 QString::number(azimuth, 'f', 1));
						if (dRatio < 1.0)
                            oss << QString(("Width of antumbra"));
						else
                            oss << QString(("Width of umbra"));
		...
		if (englishName == "Moon" && onEarth)
		{
			...
			{
                oss << QString("%1: %2%<br/>").arg(("Penumbral eclipse magnitude"), QString::number(magnitudes.first*100., 'f', 1));
				if (magnitudes.second > 1.e-3)
				{
                    oss << QString("%1: %2%<br/>").arg(("Umbral eclipse magnitude"), QString::number(magnitudes.second*100., 'f', 1));
				}
			}
		}		
		...
		if (englishName != "Sun")
            oss << QString("%1: %2<br/>").arg(("Albedo"), QString::number(getAlbedo(), 'f', 2));
	}
	return str;
}
...
void Planet::draw(StelCore* core, float maxMagLabels, const QFont& planetNameFont)
{
    ...
    if (core->getFlagPlanetMagnitudeLimit() && (getVMagnitude(core) > static_cast<float>(core->getCustomPlanetMagnitudeLimit())))
	{
    ...
    if (((getVMagnitude(core)-5.0f) > core->getLimitMagnitude()) && pType>=Planet::isAsteroid && !core->getCurrentLocation().planetName.contains("Observer", Qt::CaseInsensitive))
	{
		return;
	}
...
Vec4d Planet::getRTSTime(const StelCore *core, const double altitude) const
{
	...
	else
	{
		...
		{
			// canonical" refraction at horizon is -34'. Replace by pressure-dependent value here!
            Refraction refraction=core->getRefraction();
			Vec3d zeroAlt(1.0,0.0,0.0);
			refraction.backward(zeroAlt);
			ho += asin(zeroAlt[2]);
		}
