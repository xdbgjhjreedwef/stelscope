...
QString NomenclatureItem::getNomenclatureTypeString(NomenclatureItemType nType)
{
    ...
    return niTypeStringMap.value(nType, ("undocumented landform type; landform"));
}
...
QString NomenclatureItem::getNomenclatureTypeDescription(NomenclatureItemType nType, QString englishName)
{
    ...
    QString ret=niTypeDescriptionMap.value(nType, ("Undocumented landform type."));
	// Fix a few exceptions
	if((englishName=="Mars") && (nType==niMare))
		// TRANSLATORS: Description for landform 'mare' on Mars
        return ("'Sea'; dark albedo areas of no known geological significance.");
	if (englishName=="Titan")
	{
		if (nType==niLacus)
			// TRANSLATORS: Description for landform 'lacus' on Titan
            return ("'Lake' or small, dark plain with discrete, sharp boundaries.");
		if (nType==niMare)
			// TRANSLATORS: Description for landform 'mare' on Titan
            return ("'Sea'; large expanses of dark materials thought to be liquid hydrocarbons");
		if (nType==niSinus)
			// TRANSLATORS: Description for landform 'sinus' on Titan
            return ("'Bay'; bays within seas or lakes of liquid hydrocarbons.");
	}
	return ret;
}
...
QString NomenclatureItem::getInfoString(const StelCore* core, const InfoStringGroup& flags) const
{
	...
	if (flags&ObjectType && nType!=NomenclatureItem::niUNDEFINED)
	{
		...
		if (tstr!=latin && !latin.isEmpty() && nType<NomenclatureItemType::niSpecialPointPole)
            oss << QString("%1: <b>%2</b> (%3: %4)<br/>").arg(ts, tstr, ("geologic term"), latin);
		else
	...
	if (flags&Size && size>0. && nType<NomenclatureItem::niSpecialPointPole)
	{
		...
		if ((nType==NomenclatureItem::niCrater) || (nType==NomenclatureItem::niSatelliteFeature))
            sz = ("Diameter");
        oss << QString("%1: %2 %3<br/>").arg(sz, QString::number(size, 'f', 2), ("km ; distance"));
	}

	if (flags&Extra)
	{
        const QString cType = isPlanetocentric() ? ("Planetocentric coordinates") : ("Planetographic coordinates");
        QString sLong = StelUtils::decDegToLongitudeStr(longitude, isEastPositive(), is180(), false),
            sLat  = StelUtils::decDegToLatitudeStr(latitude, false);
		if (nType>=NomenclatureItemType::niSpecialPointEast && planet->getEnglishName()=="Jupiter")
		{
            ...
            sLong = StelUtils::decDegToLongitudeStr(360.-lng, isEastPositive(), is180(), false);
            sLat  = StelUtils::decDegToLatitudeStr(lat, false);
		}
		oss << QString("%1: %2/%3<br/>").arg(cType, sLat, sLong);
        oss << QString("%1: %2<br/>").arg(("Celestial body"), planet->getEnglishName());
		...
		if (nType!=NomenclatureItem::niUNDEFINED && nType<NomenclatureItem::niSpecialPointPole && !description.isEmpty())
            oss << QString("%1: %2<br/>").arg(("Landform description"), description);
		if (planet->getEnglishName()!="Jupiter") // we must exclude this for now due to Jupiter's "off" rotation
            oss << QString("%1: %2°<br/>").arg(("Solar altitude"), QString::number(getSolarAltitude(core), 'f', 1));
...
void NomenclatureItem::createNameLists()
{
    ...
    { niSatelliteFeature,  ("A feature that shares the name of an associated feature.")},
	// TRANSLATORS: Description for landform 'saxum'
    { niSaxum,  ("Boulder or rock.")},
	// TRANSLATORS: Description for landform 'scopulus'
    { niScopulus,  ("Lobate or irregular scarp.")},
	// TRANSLATORS: Description for landform 'serpens'
    { niSerpens,  ("Sinuous feature with segments of positive and negative relief along its length.")},
	// TRANSLATORS: Description for landform 'sinus'
    { niSinus,  ("'Bay'; small plain.")},
	// TRANSLATORS: Description for landform 'sulcus'
    { niSulcus,  ("Subparallel furrows and ridges.")},
	// TRANSLATORS: Description for landform 'terra'
    { niTerra,  ("Extensive land mass.")},
	// TRANSLATORS: Description for landform 'tessera'
    { niTessera,  ("Tile-like, polygonal terrain.")},
	// TRANSLATORS: Description for landform 'tholus'
    { niTholus,  ("Small domical mountain or hill.")},
    ...
    { niVastitas,  ("Extensive plain.")},
	// TRANSLATORS: Description for landform 'virga'
    { niVirga,  ("A streak or stripe of color.")},
	{ niLandingSite, ""},
	{ niSpecialPointPole, ""},
	{ niSpecialPointEast, ""},
	{ niSpecialPointWest, ""},
